<template>
    <header class="w-full h-fit flex flex-col items-center">
       <div class="w-full bg-red-500 py-2 flex flex-col items-center">
            <div class="w-full h-full flex-wrap max-w-[1240px] text-white text-sm flex items-center">
                <Icon icon="material-symbols-light:location-on-rounded" />
                <span class="max-md:hidden">Olaya View</span>
                <span class="border-l mx-2 h-5 block"></span>
                <!-- <Icon icon="material-symbols-light:mail" /> -->
                <span>Massage full service in saudi arabic & DOHA Qatar</span>
                <span class="border-l mx-2 h-5 block"></span>
                <Icon icon="material-symbols-light:call" />
                <a href="http://Wa.me/+966568784268">http://Wa.me/+966568784268</a>
            </div>
       </div>
       <div class="w-full bg-white max-w-[1240px] pt-2 pb-2 border-b flex items-center justify-between">
            <img class="w-[100px] h-[100px] object-cover" src="https://t4.ftcdn.net/jpg/00/99/47/49/360_F_99474971_kvwn04WzYNdXntumZ4ajbDYyfOpKxUoX.jpg" alt="">
            <ul class="max-md:hidden flex font-semibold text-gray-500 gap-5">
                <li>HOME</li>
                <li>Full Service in Doha Qatar</li>
                <li>MASSAGE YONI</li>
                <li>SERVICE</li>
                <li>TRAINNING</li>
                <li>COST</li>
                <li>CONTACT</li>
            </ul>
            <Icon class="text-3xl" icon="material-symbols-light:search-rounded" />
        </div>
    </header>
</template>